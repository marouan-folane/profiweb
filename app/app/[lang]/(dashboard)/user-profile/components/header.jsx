...
const Header = () => {
  ...
  const userAvatar = session?.user?.profileImage
    ? `${process.env.NEXT_PUBLIC_SITE_URL}${session.user.profileImage}`
    : User;
        ...
        <CardContent className="p-0">
          ...
          >
            ...
            <div className="flex items-center gap-4 absolute ltr:left-10 rtl:right-10 -bottom-2 lg:-bottom-8">
              <div>
                <div className="h-20 w-20 lg:w-32 lg:h-32 rounded-full border-4 border-background overflow-hidden relative">
                  <Image
                    src={userAvatar}
                    alt="user"
                    fill
              ...
              <div>
                <div className="text-xl lg:text-2xl font-semibold text-primary-foreground mb-1 capitalize">{userName}</div>
                <div className="text-xs lg:text-sm font-medium text-default-100 dark:text-default-900 pb-1.5 uppercase tracking-wider">{userRole}</div>
              </div>
            </div>
